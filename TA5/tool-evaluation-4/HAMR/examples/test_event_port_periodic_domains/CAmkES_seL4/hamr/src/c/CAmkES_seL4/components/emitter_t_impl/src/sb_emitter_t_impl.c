...
Unit hamr_test_event_port_periodic_domains_emitter_t_impl_seL4Nix_emit_Send(STACK_FRAME 
  art_DataContent d) {
...
void pre_init(void) {
  ...
  *sb_emit_counter = 0;
  ...
  hamr_test_event_port_periodic_domains_emitter_t_impl_adapter_initialiseArchitecture(SF_LAST);

  // call the component's initialise entrypoint
  hamr_test_event_port_periodic_domains_emitter_t_impl_adapter_initialiseEntryPoint(SF_LAST);
}
...
int run(void) {
  ...
  for(;;) {
    ...
    hamr_test_event_port_periodic_domains_emitter_t_impl_adapter_computeEntryPoint(SF_LAST);
  }
  return 0;
}
