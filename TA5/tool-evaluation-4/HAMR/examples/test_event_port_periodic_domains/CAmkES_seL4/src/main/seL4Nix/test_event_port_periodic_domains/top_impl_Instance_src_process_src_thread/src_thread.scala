...
package test_event_port_periodic_domains.top_impl_Instance_src_process_src_thread
...
import test_event_port_periodic_domains.test_event_port_periodic_domains.emitter_t_impl_seL4Nix

object src_thread extends App {

  val src_threadBridge : test_event_port_periodic_domains.test_event_port_periodic_domains.emitter_t_impl_Bridge = {
  ...
  def sendOutput(eventPortIds: ISZ[Art.PortId], dataPortIds: ISZ[Art.PortId]): Unit = {
    ...
    if(emit_port.nonEmpty) {
      emitter_t_impl_seL4Nix.emit_Send(emit_port.get)
      emit_port = noData
    }
  }
