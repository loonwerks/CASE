...
B CAmkES_seL4_VM_test_event_data_port_periodic_domains_consumer_t_impl_seL4Nix_read_port_IsEmpty(STACK_FRAME_ONLY) {
  return sb_read_port_is_empty();
}

// receive read_port: In EventDataPort union_art_DataContent
Unit CAmkES_seL4_VM_test_event_data_port_periodic_domains_consumer_t_impl_seL4Nix_read_port_Receive(STACK_FRAME
...
void pre_init(void) {
  ...
  CAmkES_seL4_VM_test_event_data_port_periodic_domains_consumer_t_impl_adapter_initialiseArchitecture(SF_LAST);

  // call the component's initialise entrypoint
  CAmkES_seL4_VM_test_event_data_port_periodic_domains_consumer_t_impl_adapter_initialiseEntryPoint(SF_LAST);
...
int run(void) {
  ...
  for(;;) {
    ...
    CAmkES_seL4_VM_test_event_data_port_periodic_domains_consumer_t_impl_adapter_computeEntryPoint(SF_LAST);
  }
  return 0;
}
