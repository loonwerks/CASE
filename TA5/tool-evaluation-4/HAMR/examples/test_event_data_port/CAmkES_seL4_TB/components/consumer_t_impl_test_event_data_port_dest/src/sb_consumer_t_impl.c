...
/************************************************************************
 *  sb_entrypoint_consumer_t_impl_test_event_data_port_dest_initializer:
 *
 * This is the function invoked by an active thread dispatcher to
 * call to a user-defined entrypoint function.  It sets up the dispatch
 * context for the user-defined entrypoint, then calls it.
 *
 ************************************************************************/
void sb_entrypoint_consumer_t_impl_test_event_data_port_dest_initializer(const int64_t * in_arg) {
...
int run(void) {
  ...
  {
    int64_t sb_dummy;
    sb_entrypoint_consumer_t_impl_test_event_data_port_dest_initializer(&sb_dummy);
  }
