...
int sp_event_port_fd;
sp_event_t_t *sp_event_port; 

seqNum_t sp_event_port_seqNum;
...
/************************************************************************
 * sp_event_port_consume:
 ************************************************************************/
bool sp_event_port_consume(void) {
  seqNum_t new_seqNum = sp_event_port_seqNum;
  if ( consume_event_t(sp_event_port, &new_seqNum) ) {
    sp_event_port_seqNum = new_seqNum;
    return true;
  } else {
    return false;
  } 
...
void pre_init(void) {
  // initialise data structure for incoming event data port event_port
  sp_event_port_seqNum = 0;
}
...
int main(int argc, char *argv[]) {
  ...
  if (argc != 5) {
    printf("Usage: %s <fd of sp_event_port> <size of sb_event_port> <fd of sb_pacer_period_queue> <size of sb_pacer_period_queue> \n\n",
           argv[0]);
    return 1;
  }

  char *sp_event_port_name = argv[1];

  int sp_event_port_length = atoi(argv[2]);
  assert(sp_event_port_length > 0);

  sp_event_port_fd = open(sp_event_port_name, O_RDWR);
  assert(sp_event_port_fd >= 0);

  char *sp_event_port_char;
  if ((sp_event_port_char = mmap(NULL, 
                                      sp_event_port_length,
                                      PROT_READ | PROT_WRITE, 
                                      MAP_SHARED, 
                                      sp_event_port_fd, 
                                      1 * getpagesize())) == (void *) -1) {
      printf("mmap sp_event_port_char failed\n");
      close(sp_event_port_fd);
      return 1;
  }

  sp_event_port = (sp_event_t_t *) sp_event_port_char;
  ...
  munmap(sp_event_port_char, sp_event_port_length);
  close(sp_event_port_fd);
