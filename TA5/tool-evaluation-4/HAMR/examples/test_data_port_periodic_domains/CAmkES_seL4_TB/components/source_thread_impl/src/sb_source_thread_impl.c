...
void sb_entrypoint_source_thread_impl_periodic_dispatcher(const int64_t * in_arg) {
...
int run(void) {
  ...
  for(;;) {
    ...
    if(sb_occurred_periodic_dispatcher){
      ...
      sb_entrypoint_source_thread_impl_periodic_dispatcher(&sb_time_periodic_dispatcher);
    }
  }
  return 0;
}
