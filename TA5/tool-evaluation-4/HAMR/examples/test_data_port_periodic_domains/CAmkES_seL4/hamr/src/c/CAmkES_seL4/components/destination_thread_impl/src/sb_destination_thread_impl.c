...
B hamr_test_data_port_periodic_domains_destination_thread_impl_seL4Nix_read_port_IsEmpty(STACK_FRAME_ONLY) {
  return sb_read_port_is_empty();
}

// receive read_port: In DataPort union_art_DataContent
Unit hamr_test_data_port_periodic_domains_destination_thread_impl_seL4Nix_read_port_Receive(STACK_FRAME
...
void pre_init(void) {
  ...
  hamr_test_data_port_periodic_domains_destination_thread_impl_adapter_initialiseArchitecture(SF_LAST);

  // call the component's initialise entrypoint
  hamr_test_data_port_periodic_domains_destination_thread_impl_adapter_initialiseEntryPoint(SF_LAST);
}
...
int run(void) {
  ...
  for(;;) {
    ...
    hamr_test_data_port_periodic_domains_destination_thread_impl_adapter_computeEntryPoint(SF_LAST);
  }
  return 0;
}
