...
object ArtNix {
  ...
  val connection: MS[Art.PortId, ISZ[(Art.PortId, Art.PortId)]] = {
    ...
    r(Arch.top_impl_Instance_src_process_src_thread.write_port.id) = ISZ(
      (IPCPorts.consumer_t_impl_dst_process_dst_thread_App, Arch.top_impl_Instance_dst_process_dst_thread.read_port.id)
    )

    r
  }
  val eventInPorts: MS[Z, Art.PortId] = MSZ(
    Arch.top_impl_Instance_dst_process_dst_thread.read_port.id
  )
