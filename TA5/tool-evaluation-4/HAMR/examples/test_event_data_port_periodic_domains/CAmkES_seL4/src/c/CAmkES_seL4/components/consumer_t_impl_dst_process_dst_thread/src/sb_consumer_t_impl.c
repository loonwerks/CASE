...
B test_event_data_port_periodic_domains_test_event_data_port_periodic_domains_consumer_t_impl_dst_process_dst_thread_seL4Nix_read_port_IsEmpty(STACK_FRAME_ONLY) {
  return sb_read_port_is_empty();
}

// receive read_port: In EventDataPort union_art_DataContent
Unit test_event_data_port_periodic_domains_test_event_data_port_periodic_domains_consumer_t_impl_dst_process_dst_thread_seL4Nix_read_port_Receive(
  STACK_FRAME
  Option_8E9F45 result) {
  DeclNewStackFrame(caller, "sb_consumer_t_impl.c", "", "test_event_data_port_periodic_domains_test_event_data_port_periodic_domains_consumer_t_impl_dst_process_dst_thread_seL4Nix_read_port_Receive", 0);
...
void pre_init(void) {
  ...
  test_event_data_port_periodic_domains_test_event_data_port_periodic_domains_consumer_t_impl_dst_process_dst_thread_adapter_initialiseArchitecture(SF_LAST);

  // call the component's initialise entrypoint
  test_event_data_port_periodic_domains_test_event_data_port_periodic_domains_consumer_t_impl_dst_process_dst_thread_adapter_initialiseEntryPoint(SF_LAST);
...
int run(void) {
  ...
  for(;;) {
    ...
    test_event_data_port_periodic_domains_test_event_data_port_periodic_domains_consumer_t_impl_dst_process_dst_thread_adapter_computeEntryPoint(SF_LAST);
    sb_self_pacer_tick_emit();
  }
  return 0;
}
