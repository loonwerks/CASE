...
object ArtNix {
  ...
  val connection: MS[Art.PortId, ISZ[(Art.PortId, Art.PortId)]] = {
    ...
    r(Arch.top_impl_Instance_producer_producer.emit.id) = ISZ(
      (IPCPorts.consumer_t_i_consumer_consumer_App, Arch.top_impl_Instance_consumer_consumer.consume.id)
    )

    r
  }
  val eventInPorts: MS[Z, Art.PortId] = MSZ(
    Arch.top_impl_Instance_consumer_consumer.consume.id
  )
