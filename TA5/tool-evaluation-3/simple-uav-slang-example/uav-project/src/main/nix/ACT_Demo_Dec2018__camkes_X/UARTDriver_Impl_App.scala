...
object UARTDriver_Impl_App extends App {
  ...
  // needed for camkes
  val WPM_tracking_idPortId: Art.PortId = Arch.UAV_Impl_Instance_MCMP_PROC_SW_WPM.tracking_id.id

  def initialise(seed: Z): Unit = {
    Platform.initialise(seed, appPortIdOpt)
    Platform.initialise(seed, mission_windowPortIdOpt)

    val i = WPM_tracking_idPortId // camkes force usage (or add getter)
