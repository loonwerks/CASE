...
void MS_E444B2_create(STACK_FRAME MS_E444B2 result, Z size, IS_82ABD8 dflt) {
  ...
  sfAssert(size <= MaxMS_E444B2, "Insufficient maximum for MS[Z, IS[Z, Z]] elements.");
...
void MS_E444B2__appendAll(STACK_FRAME MS_E444B2 result, MS_E444B2 this, MS_E444B2 other) {
  ...
  for (int8_t i = 0; i < otherSize; i++)
    Type_assign(&result->value[thisSize + i], &other->value[i], sizeof(struct IS_82ABD8));
...
void MS_E444B2_string(STACK_FRAME String result, MS_E444B2 this) {
  ...
  if (size > 0) {
    ...
    IS_82ABD8_string(SF result, (IS_82ABD8) &(value[0]));
    for (int8_t i = 1; i < size; i++) {
      String_string(SF result, string(", "));
      IS_82ABD8_string(SF result, (IS_82ABD8) &(value[i]));
    }
    String_string(SF result, space);
  }
  String_string(SF result, string("]"));
