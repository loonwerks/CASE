...
Unit HAMR_SW_Filter_Impl_Filter_Filter_seL4Nix_Output_Send(
  STACK_FRAME
  art_DataContent d) {
  DeclNewStackFrame(caller, "sb_Filter_Impl.c", "", "HAMR_SW_Filter_Impl_Filter_Filter_seL4Nix_Output_Send", 0);

  sb_Output_enqueue(d);
}

// is_empty Input: In EventDataPort
B HAMR_SW_Filter_Impl_Filter_Filter_seL4Nix_Input_IsEmpty(STACK_FRAME_ONLY) {
  return sb_Input_is_empty();
}

// receive Input: In EventDataPort union_art_DataContent
Unit HAMR_SW_Filter_Impl_Filter_Filter_seL4Nix_Input_Receive(
  STACK_FRAME
  Option_8E9F45 result) {
  DeclNewStackFrame(caller, "sb_Filter_Impl.c", "", "HAMR_SW_Filter_Impl_Filter_Filter_seL4Nix_Input_Receive", 0);
...
void pre_init(void) {
  ...
  printf("Entering pre-init of Filter_Impl_Filter_Filter\n");
  ...
  HAMR_SW_Filter_Impl_Filter_Filter_adapter_initialiseArchitecture(SF_LAST);

  // call the component's initialise entrypoint
  HAMR_SW_Filter_Impl_Filter_Filter_adapter_initialiseEntryPoint(SF_LAST);

  printf("Leaving pre-init of Filter_Impl_Filter_Filter\n");
}

void post_init(void) {
  DeclNewStackFrame(NULL, "sb_Filter_Impl.c", "", "post_init", 0);
...
int run(void) {
  ...
  for(;;) {
    ...
    HAMR_SW_Filter_Impl_Filter_Filter_adapter_computeEntryPoint(SF_LAST);
  }
  return 0;
}
