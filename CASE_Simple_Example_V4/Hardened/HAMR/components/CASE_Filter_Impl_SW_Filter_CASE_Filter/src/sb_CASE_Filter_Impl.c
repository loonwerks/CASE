...
#include <sb_CASE_Filter_Impl.h>
#include <sb_queue_union_art_DataContent_1.h>
#include <sb_event_counter.h>
#include <sb_queue_union_art_DataContent_1.h>
#include <CASE_Filter_Impl_SW_Filter_CASE_Filter_adapter.h>
#include <string.h>
#include <camkes.h>
...
// send Output: Out EventDataPort SW__RF_Msg_Impl
Unit HAMR_SW_CASE_Filter_Impl_SW_Filter_CASE_Filter_seL4Nix_Output_Send(
  STACK_FRAME
  art_DataContent d) {
  DeclNewStackFrame(caller, "sb_CASE_Filter_Impl.c", "", "HAMR_SW_CASE_Filter_Impl_SW_Filter_CASE_Filter_seL4Nix_Output_Send", 0);

  sb_Output_enqueue(d);
}

// is_empty Input: In EventDataPort
B HAMR_SW_CASE_Filter_Impl_SW_Filter_CASE_Filter_seL4Nix_Input_IsEmpty(STACK_FRAME_ONLY) {
  return sb_Input_is_empty();
}

// receive Input: In EventDataPort union_art_DataContent
Unit HAMR_SW_CASE_Filter_Impl_SW_Filter_CASE_Filter_seL4Nix_Input_Receive(
  STACK_FRAME
  Option_8E9F45 result) {
  DeclNewStackFrame(caller, "sb_CASE_Filter_Impl.c", "", "HAMR_SW_CASE_Filter_Impl_SW_Filter_CASE_Filter_seL4Nix_Input_Receive", 0);
...
void pre_init(void) {
  DeclNewStackFrame(NULL, "sb_CASE_Filter_Impl.c", "", "pre_init", 0);

  printf("Entering pre-init of CASE_Filter_Impl_SW_Filter_CASE_Filter\n");
  ...
  HAMR_SW_CASE_Filter_Impl_SW_Filter_CASE_Filter_adapter_initialiseArchitecture(SF_LAST);

  // call the component's initialise entrypoint
  HAMR_SW_CASE_Filter_Impl_SW_Filter_CASE_Filter_adapter_initialiseEntryPoint(SF_LAST);

  printf("Leaving pre-init of CASE_Filter_Impl_SW_Filter_CASE_Filter\n");
}

void post_init(void) {
  DeclNewStackFrame(NULL, "sb_CASE_Filter_Impl.c", "", "post_init", 0);
 ...
 ************************************************************************/
int run(void) {
  DeclNewStackFrame(NULL, "sb_CASE_Filter_Impl.c", "", "run", 0);

  MUTEXOP(sb_dispatch_sem_wait())
  for(;;) {
    MUTEXOP(sb_dispatch_sem_wait())
    // call the component's compute entrypoint
    HAMR_SW_CASE_Filter_Impl_SW_Filter_CASE_Filter_adapter_computeEntryPoint(SF_LAST);
  }
  return 0;
}
