...
@datatype class CASE_AttestationGate_Impl_Operational_Api (
  ...
  def get_TrustedIds() : Option[SW.Address_Impl] = {
    val value : Option[SW.Address_Impl] = Art.getValue(TrustedIds_Id) match {
      case Some(SW.Address_Impl_Payload(v)) => Some(v)
      case Some(v) =>
        Art.logError(id, s"Unexpected payload on port TrustedIds.  Expecting 'SW.Address_Impl_Payload' but received ${v}")
        None[SW.Address_Impl]()
