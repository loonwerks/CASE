...
abstract class AttestationGate_Impl_AttestationGate_AttestationGate_TestApi extends BridgeTestSuite[AttestationGate_Impl_AttestationGate_AttestationGate_Bridge](Arch.SW_seL4_Impl_Instance_AttestationGate_AttestationGate) {
  ...
  def put_concrete_inputs(MissionCommand_in : ISZ[SW.RF_Msg_Impl],
                          TrustedIds : ISZ[SW.AllowList_Impl]): Unit = {
  ...
  /** helper function to check AttestationGate_Impl_AttestationGate_AttestationGate's
  ...
  def put_TrustedIds(value : SW.AllowList_Impl): Unit = {
    ArtNative_Ext.insertInPortValue(bridge.operational_api.TrustedIds_Id, SW.AllowList_Impl_Payload(value))
  }
