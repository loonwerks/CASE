...
extern dataport_caps_handle_t sb_MissionCommand_queue_handle;
seL4_Word sb_MissionCommand_notification_notification_badge(void);
extern dataport_caps_handle_t sb_FlightPlan_queue_1_handle;
void sb_FlightPlan_1_notification_emit_underlying(void);
...
void init_cross_vm_connections(vm_t *vm, void *cookie) {
    connections[0] = (struct camkes_crossvm_connection) {
      .handle = &sb_MissionCommand_queue_handle,
      .emit_fn = NULL,
      .consume_badge = sb_MissionCommand_notification_notification_badge(),
      .connection_name = "sb_MissionCommand_queue"
    };

    connections[1] = (struct camkes_crossvm_connection) {
      .handle = &sb_FlightPlan_queue_1_handle,
      .emit_fn = sb_FlightPlan_1_notification_emit_underlying,
      .consume_badge = -1,
      .connection_name = "sb_FlightPlan_queue_1"
    };
