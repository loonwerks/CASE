...
Unit CASE_MONITOR_2_TEST_VPM_TEST_Input_impl_Input_Input_seL4Nix_Observed_Send(
  STACK_FRAME
  art_DataContent d) {
  DeclNewStackFrame(caller, "sb_Input_impl.c", "", "CASE_MONITOR_2_TEST_VPM_TEST_Input_impl_Input_Input_seL4Nix_Observed_Send", 0);
...
void pre_init(void) {
  ...
  CASE_MONITOR_2_TEST_VPM_TEST_Input_impl_Input_Input_adapter_initialiseArchitecture(SF_LAST);

  // call the component's initialise entrypoint
  CASE_MONITOR_2_TEST_VPM_TEST_Input_impl_Input_Input_adapter_initialiseEntryPoint(SF_LAST);
...
int run(void) {
  ...
  for(;;) {
    ...
    CASE_MONITOR_2_TEST_VPM_TEST_Input_impl_Input_Input_adapter_computeEntryPoint(SF_LAST);
    sb_self_pacer_tick_emit();
  }
  return 0;
}
