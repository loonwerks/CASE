...
Unit CASE_MONITOR_2_TEST_VPM_CASE_Monitor1_Impl_CASE_Monitor_CASE_Monitor_seL4Nix_Suspect_Traffic_Out_Send(
  STACK_FRAME
  art_DataContent d) {
  DeclNewStackFrame(caller, "sb_CASE_Monitor1_Impl.c", "", "CASE_MONITOR_2_TEST_VPM_CASE_Monitor1_Impl_CASE_Monitor_CASE_Monitor_seL4Nix_Suspect_Traffic_Out_Send", 0);

  sb_Suspect_Traffic_Out_enqueue(d);
}

// is_empty Observed: In EventDataPort
B CASE_MONITOR_2_TEST_VPM_CASE_Monitor1_Impl_CASE_Monitor_CASE_Monitor_seL4Nix_Observed_IsEmpty(STACK_FRAME_ONLY) {
  return sb_Observed_is_empty();
}

// receive Observed: In EventDataPort union_art_DataContent
Unit CASE_MONITOR_2_TEST_VPM_CASE_Monitor1_Impl_CASE_Monitor_CASE_Monitor_seL4Nix_Observed_Receive(
  STACK_FRAME
  Option_8E9F45 result) {
  DeclNewStackFrame(caller, "sb_CASE_Monitor1_Impl.c", "", "CASE_MONITOR_2_TEST_VPM_CASE_Monitor1_Impl_CASE_Monitor_CASE_Monitor_seL4Nix_Observed_Receive", 0);
...
void pre_init(void) {
  ...
  printf("Entering pre-init of CASE_Monitor1_Impl_CASE_Monitor_CASE_Monitor\n");
  ...
  CASE_MONITOR_2_TEST_VPM_CASE_Monitor1_Impl_CASE_Monitor_CASE_Monitor_adapter_initialiseArchitecture(SF_LAST);

  // call the component's initialise entrypoint
  CASE_MONITOR_2_TEST_VPM_CASE_Monitor1_Impl_CASE_Monitor_CASE_Monitor_adapter_initialiseEntryPoint(SF_LAST);

  printf("Leaving pre-init of CASE_Monitor1_Impl_CASE_Monitor_CASE_Monitor\n");
}
...
int run(void) {
  ...
  for(;;) {
    ...
    CASE_MONITOR_2_TEST_VPM_CASE_Monitor1_Impl_CASE_Monitor_CASE_Monitor_adapter_computeEntryPoint(SF_LAST);
    sb_self_pacer_tick_emit();
  }
  return 0;
}
