...
B CASE_MONITOR2_TEST_VPM_TEST_Output_impl_Output_Output_seL4Nix_Suspect_Traffic_Out_IsEmpty(STACK_FRAME_ONLY) {
  return sb_Suspect_Traffic_Out_is_empty();
}

// receive Suspect_Traffic_Out: In EventDataPort union_art_DataContent
Unit CASE_MONITOR2_TEST_VPM_TEST_Output_impl_Output_Output_seL4Nix_Suspect_Traffic_Out_Receive(
  STACK_FRAME
  Option_8E9F45 result) {
  DeclNewStackFrame(caller, "sb_Output_impl.c", "", "CASE_MONITOR2_TEST_VPM_TEST_Output_impl_Output_Output_seL4Nix_Suspect_Traffic_Out_Receive", 0);
...
void pre_init(void) {
  ...
  CASE_MONITOR2_TEST_VPM_TEST_Output_impl_Output_Output_adapter_initialiseArchitecture(SF_LAST);

  // call the component's initialise entrypoint
  CASE_MONITOR2_TEST_VPM_TEST_Output_impl_Output_Output_adapter_initialiseEntryPoint(SF_LAST);
...
int run(void) {
  ...
  for(;;) {
    ...
    CASE_MONITOR2_TEST_VPM_TEST_Output_impl_Output_Output_adapter_computeEntryPoint(SF_LAST);
    sb_self_pacer_tick_emit();
  }
  return 0;
}
