...
Unit HAMR_Simple_V4_SW_FlightPlanner_Impl_SW_FlightPlanner_FlightPlanner_seL4Nix_FlightPlan_Send(
  STACK_FRAME
  art_DataContent d) {
  DeclNewStackFrame(caller, "sb_FlightPlanner_Impl.c", "", "HAMR_Simple_V4_SW_FlightPlanner_Impl_SW_FlightPlanner_FlightPlanner_seL4Nix_FlightPlan_Send", 0);

  sb_FlightPlan_enqueue(d);
}

// is_empty MissionCommand: In EventDataPort
B HAMR_Simple_V4_SW_FlightPlanner_Impl_SW_FlightPlanner_FlightPlanner_seL4Nix_MissionCommand_IsEmpty(STACK_FRAME_ONLY) {
  return sb_MissionCommand_is_empty();
}

// receive MissionCommand: In EventDataPort union_art_DataContent
Unit HAMR_Simple_V4_SW_FlightPlanner_Impl_SW_FlightPlanner_FlightPlanner_seL4Nix_MissionCommand_Receive(
  STACK_FRAME
  Option_8E9F45 result) {
  DeclNewStackFrame(caller, "sb_FlightPlanner_Impl.c", "", "HAMR_Simple_V4_SW_FlightPlanner_Impl_SW_FlightPlanner_FlightPlanner_seL4Nix_MissionCommand_Receive", 0);
...
void pre_init(void) {
  ...
  HAMR_Simple_V4_SW_FlightPlanner_Impl_SW_FlightPlanner_FlightPlanner_adapter_initialiseArchitecture(SF_LAST);

  // call the component's initialise entrypoint
  HAMR_Simple_V4_SW_FlightPlanner_Impl_SW_FlightPlanner_FlightPlanner_adapter_initialiseEntryPoint(SF_LAST);
...
int run(void) {
  ...
  for(;;) {
    ...
    HAMR_Simple_V4_SW_FlightPlanner_Impl_SW_FlightPlanner_FlightPlanner_adapter_computeEntryPoint(SF_LAST);
  }
  return 0;
}
