...
object FlightController_Impl_SW_FlightController_App extends App {
  ...
  val AlertPortId: Art.PortId = Arch.MissionComputer_Impl_Instance_SW_FlightController.Alert.id
  val AlertPortIdOpt: Option[Art.PortId] = Some(AlertPortId)

  def initialiseArchitecture(seed: Z): Unit = {
    Platform.initialise(seed, appPortIdOpt)
    Platform.initialise(seed, FlightPlanPortIdOpt)
    Platform.initialise(seed, AlertPortIdOpt)
  ...
  def compute(): Unit = {
    ...
    {
      val out = IPCPorts.emptyReceiveAsyncOut
      Platform.receiveAsync(AlertPortIdOpt, out)
      out.value2 match {
        case Some(v: art.Empty) => ArtNix.updateData(AlertPortId, v); dispatch = T
        case Some(v) => halt(s"Unexpected payload on port Alert.  Expecting something of type art.Empty but received ${v}")
        case None() => // do nothing
      }
    }
    if (dispatch) {
  ...
  def touch(): Unit = {
    if(F) {
      ...
      val apiUsage_Alert: Option[art.Empty] = HAMR_Simple_V4.SW.FlightController_Impl_SW_FlightController_Bridge.c_operational_api.get.get_Alert()
    }
  }
